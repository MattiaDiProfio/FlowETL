...
def compute_schema_map(ti):
    ...
    prompt = f"""
    Find a plausible column mapping between the following source and target column headers. 
    Note that at most two attributes from the source columns can be merged into one target column 
    and a source column can be decomposed into at most two target columns. Return the output in a 
    python dictionary {{ (s1 , s2) : (t1, t2) }}. Source = {source_headers}, Target = {target_headers}. 
    Note that some source attributes could be dropped by the mapping, while some target attributes created.
    Only return valid python code, no comments, and do not justify your answer.
    """
    ...
    if response.status_code == 200:
        ...
        schema_map = extract_code(response_data['choices'][0]['message']['content'])
